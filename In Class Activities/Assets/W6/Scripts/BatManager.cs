...
public class BatManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < _bats.Length; i++) {
            Vector3 _batsPosition = _bats[i].gameObject.transform.position;
            Vector3 _playerPosition = _playerTransform.position;
            if (Mathf.Abs(Vector3.Distance(_playerPosition, _batsPosition)) <= _overlapDistance)
            {
                Debug.Log("Creating Reactions");
                CreateReactions(_bats[i]);
            }
            else if (Mathf.Abs(Vector3.Distance(_playerPosition, _batsPosition)) <= _interactDistance)
            {
                Debug.Log("Enabling Chase");
    ...
    private void CreateReactions(BatW6 bat)
    {
        ...
        //      of CreateReactions.
        int randomIdx = Random.Range(0, _messages.Length);
        SpawnReactionUI(bat, _messages[randomIdx]);
